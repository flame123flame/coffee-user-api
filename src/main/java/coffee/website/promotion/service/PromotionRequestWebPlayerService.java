...
@Service
public class PromotionRequestWebPlayerService {
    @Autowired
    ...
    public List<PromotionRequest> promotionPlayerHistory(String username) {
        return promotionRequestJpa.findByUsername(username);
    }

    public void recivePromotion(PromotionRequestReq req) {
        String username = req.getUsername();
        String requestId = GenerateRandomString.generateUUID();
        CustomerRes customer = customerService.getCustomerByUsername(username);
        List<DepositMainRes> deposit = depositService.getDepositByUsernameDesc(username);
        Promotion promotion = promotionService.getByPromoCode(req.getPromoCode());
        AppSetting appSetting = promotionService.getAppSettingByPromoCode(promotion.getPromoCode());
        RuleSetting ruleSetting = promotionService.getRuleSettingByPromoCode(promotion.getPromoCode());

        PromotionRequest data = new PromotionRequest();
        data.setPlayerGroup(customer.getGroupName());
        data.setUsername(username);
        data.setRequestId(requestId);
        data.setPromoName(promotion.getPromoTitle());
        data.setPromoType(promotion.getPromoType());
        data.setPromoCode(req.getPromoCode());
        data.setCreatedBy(username);

        StatusDetail rejectCondition = rejectCondition(promotion, req, customer, appSetting, ruleSetting, deposit);
        if (rejectCondition.getCheckCondition()) {
            data.setStatusRequest(ProjectConstant.STATUS.PENDING);
            BigDecimal resultAmount = BigDecimal.ZERO;
            DepositMainRes targetBalance = null;

            if (PromotionConstant.Type.registration.equals(promotion.getPromoType())) {
                resultAmount = calculateByRegistration(promotion, username, ruleSetting, requestId);
            }
            if (PromotionConstant.Type.firstAndSecondDeposit.equals(promotion.getPromoType())) {
                List<PromotionMapping> mapping = new ArrayList<PromotionMapping>();
                targetBalance = filterTargetDeposit(promotion, appSetting, deposit, mapping);
                BigDecimal depositBalance = new BigDecimal(targetBalance.getAmount());
                BonusLevelSetting bonusSetting = promotionService.findLevelBonus(req.getPromoCode(), depositBalance);
                data.setBonusLevel(bonusSetting.getBonusLevel());

                resultAmount = calculateByFisrtAndSecond(promotion, username, appSetting, ruleSetting, bonusSetting, depositBalance, requestId);
            }
            if (PromotionConstant.Type.generalDeposit.equals(promotion.getPromoType())) {
                targetBalance = deposit.get(0);
                BigDecimal depositBalance = new BigDecimal(targetBalance.getAmount());
                BonusLevelSetting bonusSetting = promotionService.findLevelBonus(req.getPromoCode(), depositBalance);
                data.setBonusLevel(bonusSetting.getBonusLevel());

                resultAmount = calculateByGeneralDeposit(promotion, username, appSetting, ruleSetting, bonusSetting, depositBalance, requestId);
            }

//            if(ruleSetting.getSingleBonusLimit() != null) {
//            	if(resultAmount.compareTo(ruleSetting.getSingleBonusLimit()) > 0) {
//            		resultAmount = ruleSetting.getSingleBonusLimit();
//            	}
//            }

            if (PromotionConstant.WalletType.BALANCE.equals(ruleSetting.getReceiveBonusWallet())) {
                data.setBalanceAmount(resultAmount);
                data.setBonusAmount(BigDecimal.ZERO);
            } else {
                data.setBalanceAmount(BigDecimal.ZERO);
                data.setBonusAmount(resultAmount);
            }
        } else {
            data.setBalanceAmount(BigDecimal.ZERO);
            data.setBonusAmount(BigDecimal.ZERO);
            data.setStatusRequest(ProjectConstant.STATUS.REJECT);
            data.setRemark(rejectCondition.getRemark());
            data.setUpdatedAt(new Date());
            data.setUpdatedBy("_system");
        }
        WebSocketNotificationRes responseString = new WebSocketNotificationRes();
        responseString.setType("PROMOTION_REQUEST");
        responseString.setMessage(req.getUsername() + " request promotion.");
        responseString.setUsername(req.getUsername());
        this.simpMessagingTemplate.convertAndSend("/admin/notification/promotion", responseString);
        promotionRequestJpa.save(data);

        if (PromotionConstant.VerificationType.Auto.equals(appSetting.getVerificationType())) {

            PromotionRequestReq request = new PromotionRequestReq();
            request.setRequestId(requestId);
            request.setPlayerGroup(customer.getGroupCode());
            request.setPromoCode(req.getPromoCode());
            request.setUsername(username);
            request.setRequestId(requestId);

            if (rejectCondition.getCheckCondition()) {
                request.setStatusRequest(ProjectConstant.STATUS.APPROVE);
                changeStatusPromotion(request);
            } else {
                request.setStatusRequest(ProjectConstant.STATUS.REJECT);
                changeStatusPromotion(request);
            }
        }
    }
    ...
    public StatusDetail rejectCondition(Promotion promotion, PromotionRequestReq req, CustomerRes customer, AppSetting appSetting, RuleSetting ruleSetting, List<DepositMainRes> deposit) {
        ...
        boolean checkReciveLimit = false;
    ...
    public BigDecimal calculateByGeneralDeposit(Promotion promotion, String username, AppSetting appSetting, RuleSetting ruleSetting, BonusLevelSetting bonusSetting, BigDecimal depositBalance, String requestId) {
        ...
        if(ruleSetting.getSingleBonusLimit() != null) {
        	if(result.compareTo(ruleSetting.getSingleBonusLimit()) > 0) {
        		result = ruleSetting.getSingleBonusLimit();
        	}
        }
